...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series INNER JOIN characters ON characters.series_id = series.id GROUP BY characters.species HAVING characters.species = 'human' ORDER BY COUNT(characters.species) desc LIMIT 1; "
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name , COUNT(character_books.book_id AND character_books.character_id) from characters INNER JOIN character_books ON characters.id = character_books.book_id GROUP BY ;"
end
